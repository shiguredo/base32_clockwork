...
- 設計方針の「全ビット数が 8 の倍数であることを検証する」は、RFC 4648 で有効なパディングなし長 (mod 8 = 2, 4, 5, 7) もすべて拒否してしまう。
- パディングなし入力を (A) パディング必須として拒否するか、(B) 正当長を切詰めとパディングビットのゼロ検証で受理するかの方針決定が必要であり、完了条件・実装量・変更種別が変わるため保留する。
